...
def write_to_record(id_batch, label_batch, predictions, filenum, num_examples_processed, top_k, whitelisted_cls_mask):
    writer = tf.io.TFRecordWriter(FLAGS.output_dir + '/' + 'predictions-%04d.tfrecord' % filenum)
    for i in range(num_examples_processed):
        video_id = id_batch[i]
        #label = np.nonzero(label_batch[i,:])[0]     ##out of bound? when use test       
        example = get_output_feature(video_id, [predictions[i,:]], ['predictions'])
        serialized = example.SerializeToString()
        writer.write(serialized)
    writer.close()
def get_output_feature(video_id, features, feature_names):
    video_id = tf.compat.as_bytes(video_id)
  ...
  with tf.Session(config=tf.ConfigProto(
        ...
        with tf.io.gfile.GFile(segment_label_ids_file) as fobj:
          for line in fobj:
            try:
              ...
              continue
    print(video_id_batch, video_batch, num_frames_batch)
    ...
    try:
      while not coord.should_stop():
        ...
        print("num_frames_batch_val = ", num_frames_batch_val)
        if FLAGS.segment_labels:
        ...
        print(a)
        a += 1
        #print("video_id_batch_val = ",video_id_batch_val)
        ...
        for line in format_lines(video_id_batch_val, predictions_val, top_k, whitelisted_cls_mask):
            out_file.write(line)
        if num_examples_processed >= FLAGS.file_size:
            ...
            write_to_record(video_id, video_label, video_features, filenum, num_examples_processed, top_k, whitelisted_cls_mask)
            filenum += 1
            video_id = []
            video_label = []
            video_features = []
