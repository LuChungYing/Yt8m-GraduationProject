...
def evaluate():
  ...
  with tf.Graph().as_default():
    ...
    inference(saver,reader, FLAGS.train_dir, 
...
def write_to_record(id_batch, label_batch, predictions, filenum, num_examples_processed):
    ...
    for i in range(num_examples_processed):
        if(i < num_examples_processed-1 ):
            if id_batch[i] == id_batch[i+1]:
                a += 1
            else:
                a = 0
        else:
            a += 1
        id_batch[i] = id_batch[i] +":" +str(a) 
...
#################################3
def inference(saver, reader, model_checkpoint_path, out_file_location, batch_size, top_k):
  with tf.Session() as sess:
    ...
    video_id_batch, video_batch, num_frames_batch, labels_batch = get_input_data_tensors(
        reader, FLAGS.input_data_pattern, batch_size)
    ...
    try:
      while not coord.should_stop():
          ...
          num_frames_batch_val = sess.run(num_frames_batch)
        ...
        if 0 < num_examples_processed <= FLAGS.file_size:
            ...
            video_features = np.concatenate(video_features,axis=0)
            write_to_record(video_id, video_label, video_features, filenum,num_examples_processed)
    print("Finish writing to tfrecord")
    coord.join(threads)
    sess.close()
